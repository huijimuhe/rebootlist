...
class DayDart {
   ...
   */
  String tillStr() {
    var now = new DateTime.now(); // 开始时间
    var interval = (now.difference(_date).inSeconds).abs(); //这个是秒
    var d = (interval / 3600 / 24).floor();
    var h = ((interval - 3600 * 24 * d) / 3600).floor();
    var m = ((interval - 3600 * 24 * d) % 3600 / 60).floor();
    return d == 0
        ? h == 0 ? m < 1 ? '刚刚' : '$m分钟后' : '$h小时$m分钟后'
        : d < 3 ? '$d天后' : toISOString();
  }
  ...
  int toSec() {
    var now = new DateTime.now(); // 开始时间
    return (_date.difference(now).inSeconds).abs(); //这个是秒
  }
  ...
  int fromSec() {
    var now = new DateTime.now(); // 开始时间
    return (now.difference(_date).inSeconds).abs(); //这个是秒
  }
  ...
  String fromStr() {
    var now = new DateTime.now(); // 开始时间
    var interval = (_date.difference(now).inSeconds).abs(); //这个是秒
    var d = (interval / 3600 / 24).floor();
    var h = ((interval - 3600 * 24 * d) / 3600).floor();
    var m = ((interval - 3600 * 24 * d) % 3600 / 60).floor();
    return d == 0
        ? h == 0 ? m < 1 ? '刚刚' : '$m分钟前' : '$h小时$m分钟前'
        : d < 3 ? '$d天后' : toISOString();
  }
  ...
  String format({String fm = FORMAT_DEFAULT}) {
    ...
    return fm.replaceAllMapped(
        new RegExp(
            r'\[.*?\]|Y{2,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS'),
        (Match m) {
