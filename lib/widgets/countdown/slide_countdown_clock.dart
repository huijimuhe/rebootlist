...
part 'package:rebootlist/widgets/countdown/clip_digit.dart';

part 'package:rebootlist/widgets/countdown/digit.dart';

part 'package:rebootlist/widgets/countdown/slide_direction.dart';
...
class SlideCountdownClock extends StatefulWidget {
  ...
  /// A callback that is called when the [duration] reaches 10.
  ...
  void _init() {
    ...
    initStream = Stream<DateTime>.periodic(Duration(milliseconds: 1000), (_) {
      ...
      if (timeLeft.inSeconds == 0) {
        ...
        if (widget.onAlmostDone != null) widget.onAlmostDone();
      }
      return time;
    });
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: <Widget>[
        _buildDigit(
          timeStream,
          (DateTime time) => (timeLeft.inHours) ~/ 10,
          (DateTime time) => (timeLeft.inHours) % 10,
          DateTime.now(),
          "Hours",
        ),
